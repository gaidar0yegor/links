...
class ContentGenerator:
    ...
    def get_fallback_template(self) -> Dict[str, str]:
        ...
        return {
            'text': '{product_name}\n\n⚫️Corpo sottile dal design minimalista, perfetto per ogni ambiente\n\n⚫️ Due modalità di illuminazione — luce calda e fredda — ideale per lavoro, lettura o relax\n\n⚫️ Batteria integrata: utilizzabile senza fili, comodissima sul comodino o in viaggio\n\n⚫️ Regolatore touch della luminosità — ajustabile con un solo tocco\n\n💵 Prezzo: {price}\n\n⭐️ Rating: {rating}/5\n\n🔜 Affrettati e ordina ora: {affiliate_link}\n\n#ad Come partner Amazon ricevo una commissione sugli acquisti idonei',
        }
    ...
    async def generate_content(self, product_data: Dict[str, Any], category: Optional[str] = None, language: str = 'en') -> Optional[Dict[str, str]]:
        ...
        """
        try:
            # If category parameter is provided, use fallback template
            if category:
                template = self.get_fallback_template()
            else:
                # Determine category from product_data
